...
app.post("/create-order", (req, res) => {
    ...
    const merchantTradeNo =
        "NBA" + Date.now() + Math.floor(Math.random() * 1000);
...
// 為了與 Vercel API 路由兼容，添加 /api/ 前綴的路由
// 使用動態 import 來載入 ES 模組
app.post("/api/create-order", async (req, res) => {
    try {
        const { default: createOrderHandler } = await import(
            "./api/create-order.js"
        );
        createOrderHandler(req, res);
    } catch (error) {
        console.error("Error loading create-order API:", error);
...
app.post("/api/ecpay-return", async (req, res) => {
    try {
        const { default: ecpayReturnHandler } = await import(
            "./api/ecpay-return.js"
        );
        ecpayReturnHandler(req, res);
    } catch (error) {
        console.error("Error loading ecpay-return API:", error);
        res.status(500).json({ error: "內部伺服器錯誤" });
    }
});

app.all("/api/ecpay-order-result", async (req, res) => {
    try {
        const { default: ecpayOrderResultHandler } = await import(
            "./api/ecpay-order-result.js"
        );
        ecpayOrderResultHandler(req, res);
    } catch (error) {
        console.error("Error loading ecpay-order-result API:", error);
        res.status(500).json({ error: "內部伺服器錯誤" });
    }
});
...
app.get("/api/payment-success", async (req, res) => {
    try {
        const { default: paymentSuccessHandler } = await import(
            "./api/payment-success.js"
        );
        paymentSuccessHandler(req, res);
    } catch (error) {
        console.error("Error loading payment-success API:", error);
        res.status(500).json({ error: "內部伺服器錯誤" });
    }
});
...
app.listen(PORT, () => {
    ...
    console.log("API 路由已啟用，與 Vercel 相容");
});
